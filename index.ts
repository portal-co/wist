...
export class WebSocket extends EventTarget {
  ...
  async #start() {
    const {
      _fetch: fetch = _fetch,
      intOps: _intOps = intOps,
      _Promise: Promise = _Promise,
      _ArrayBuffer: ArrayBuffer = _ArrayBuffer,
      _Uint8Array: Uint8Array = _Uint8Array,
      _DataView: DataView = _DataView,
      _slice: slice = _slice,
      byteLength: _byteLength = byteLength,
      _requestAnimationFrame: requestAnimationFrame = _requestAnimationFrame,
      _URL: URL = _URL,
    } = this.#opts;
    ...
    while (1) {
      ...
      while (i != responseLength) {
        ...
        if (packetType & 1) {
          this.dispatchEvent(
            new MessageEvent("message", { data: decode(byteData) })
          );
        } else {
...
export class Server {
  ...
  async handle(r: Request): Promise<Response> {
    ...
    user.onBuffer(
      this.#proc(new Uint8Array(await r.arrayBuffer()), "decrypt").buffer
    );
    expiry.set(user, Date.now() + this.#expiry);
    // this.#ids[s] = [];
    return new Response(
      this.#proc(
        new Uint8Array(s.reduce(appendBuffers, new ArrayBuffer(0))),
        "encrypt"
      )
    );
    // }
